...
7. «مشاهده اطلاعات نسخه نرم‌افزار» شامل بررسی خودکار و بی‌صدای نسخه جدید در Startup است (استثنای مجاز از قانون Offline-First — به `ARCHITECTURE.md` بخش «سیاست دسترسی به شبکه» مراجعه شود)؛ کاربر می‌تواند این بررسی خودکار را از همین بخش خاموش کند و فقط دستی («بررسی به‌روزرسانی») استفاده کند.
