...
## Audit closure 2026-09-19 (fcb6fb2 follow-up)
- BUG-P0-001: report composition moved to `src/application/reporting/reportPack.js`; Core no longer imports Features
- BUG-P0-002: Node offline harness isolated under `src/platform/node/`; browser shipping path documented as not implemented
- BUG-P0-003: coding.ui = GUARDED_IMPLEMENTATION_ALLOWED (shell scaffold; React product host still pending)
- Investment totals: mixed cost currencies without reportCurrency → null totals + flag
- Rebuild: journal lines loaded from relational tables; result_json optional only

## Contract locks (2026-09-19 P1)
- **assets.dispose** = financial disposal with journal (proceeds + inventory release) — not metadata-only
- **R-M22** = PARTIAL (import pipeline local; full release gate open)
- **coding.ui** = CONTROLLED_IMPLEMENTATION
- **applicationApi** = full module surface for web host

## Money vertical slice (2026-09-20)
- book.create + book.get via hostGateway
- Account list VM journal-derived balances
- Account create / deposit / withdraw / transfer sheets wired to host commands
- E2E: book → create account → deposit 10M → list → idempotent retry → backup → restore
- backupDatabase no longer renames live SQLite under open connection
- PRODUCTION still NO-GO; browser sql.js+IDB RELEASE E2E still OPEN

## Coding wave 2026-09-20 (transactions/loan/invest/host)
- dashboardSummary + listCheques host queries (mixed currency → no invented total)
- Investments E2E: crypto.buy → listCryptoHoldings
- Cheque E2E: register wired fail-closed
- React gateway JS twin + FinancialHost bridge E2E (money + dashboard)
- Onboarding calls book.create when host bound
- HomeScreen metrics from dashboardSummary / listLoans (missing ≠ 0)
- PRODUCTION NO-GO; browser IDB release E2E still OPEN

## P0 customer-readiness (2026-09-20)
- P0-01: bootstrapRuntime + AppProviders bind host via setFinancialHost / window.__PF_HOST__; phases bootstrapping|awaiting_host|onboarding|ready|recovery
- P0-02: localStorage only UI prefs; book readiness from host.query(book.get) after book.create
- P0-03: singleWriter remote lease + navigator.locks path; dual-token rejection tested
- P0-04: createFinancialHost enforces isCommandAllowed(edition); queries remain readable
- RELEASE still NO-GO until real sql.js+IDB browser E2E (R-M24)

## P0-03..06 closure (2026-09-20)
- P0-03: browserProductionBoot.js separated from Node sqlJsIndexedDbAdapter harness; waves A–C IMPLEMENTED (not PROVEN)
- P0-04: singleWriter = navigator.locks + IDB lease + BC notify; dual-token tests green
- P0-05: FinancialHost assertEditionAllows already on main (LICENSE_REQUIRED)
- P0-06: resolveMoneyOperationFx on deposit/withdraw/transfer/income/expense/cheque.clear/tax/assets.dispose

## P0-07 + P1 UI (2026-09-20)
- Loan: withinTransaction uses ensureLoanAccounts(db)/ensureLocalSettlementAccounts(db) — no bootstrapLoanEditionAccounts(dataDir)
- cheque register/deposit/cancel + tax.adjust: withImmediateTransaction
- listAccounts.totalsByCurrency (1+2+3m IRR = 6m)
- AccountSelect on deposit/withdraw/transfer
- TransactionDetailSheet + reverse; LoanReverseSheet; LoanCreate full domain + previewSchedule

## P1-06..18 product surfaces (2026-09-20)
- listInvestmentHoldings + InvestmentsScreen unified tabs/valuationState
- Investment sheets: crypto/stocks/funds/metals real forms (reinvest deferred)
- ReportsPanel runs trialBalance/reportPack/holdings via gateway
- ImportExportSheet phases + import.ingestBatch; BackupSheet blob download + SQLite magic refuse
- tax.adjust audited (changeId, old/new, fin_audit_log); cheque state machine exact tests
>>>>>>> origin/main
