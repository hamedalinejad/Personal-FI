...
- گزارش‌ها باید سریع باشند؛ در صورت نیاز از Snapshot و کش استفاده شود.
- تمام مبالغ قابلیت نمایش به ریال و معادل تتری (با نرخ تاریخی) را داشته باشند.
- فیلترهای مشترک (بازه زمانی، حساب، دسته) در همه گزارش‌ها یکسان باشد.
- نمودارها باید ساده، خوانا و مناسب موبایل باشند.
- این فیچر نباید منطق کسب‌وکار سایر فیچرها را تکرار کند؛ فقط تجمیع و نمایش دهد.
- Job روزانه برای ثبت Net Worth Snapshot پیشنهاد می‌شود تا روند تاریخی دقیق بماند.