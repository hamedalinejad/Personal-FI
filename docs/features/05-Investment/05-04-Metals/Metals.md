...
موجودی فلز افزایش و میانگین خرید به‌روزرسانی می‌شود.

فروش فلز:
موجودی فلز کاهش می‌یابد.
مبلغ حاصل به موجودی نقدی پلتفرم اضافه می‌شود.

تحویل فیزیکی:
موجودی دیجیتال فلز کاهش می‌یابد (از موجودی پلتفرم خارج می‌شود).
هزینه تحویل/ضرب در صورت وجود ثبت می‌شود.
وضعیت درخواست تحویل پیگیری می‌شود (requested, processing, delivered, cancelled).
...
minBuyAmount → decimal (حداقل مبلغ خرید — ریال)
...
۳. Metals Transaction (جدول: metals_transactions) — لاگ خرید و فروش
...
deliveryAddress → string / یا شعبه
...
requestPhysicalDelivery(data) → ثبت درخواست تحویل فیزیکی
...
تحویل فیزیکی موجودی دیجیتال را کاهش می‌دهد و در صورت تمایل می‌تواند به فیچر دارایی‌های فیزیکی لینک شود.
