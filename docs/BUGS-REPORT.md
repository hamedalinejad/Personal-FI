...
- افزودن فیلدهای `priceBase`/`totalAmountBase` به `inv_crypto_transactions` (معادل هر تراکنش به ارز پایه، با استفاده از `exchangeRateToBase` یا `getLatestPrice()` از فیچر `19-Price-Fetching`)، دقیقاً مطابق الگوی موجود `feeAssetPriceToUSDT`.
...
هیچ باگ منطقی/محاسباتی بزرگی در فرمول‌های حسابداری یا وام پیدا نشد. در بخش Investment-Crypto اما یک باگ محاسباتی واقعی (باگ ۷ بالا — عدم پشتیبانی درست از خرید/فروش با ارزهای مختلف) در بررسی بعدی کشف و رفع شد. باقی باگ‌های پیداشده از نوع «Drift بین اسناد» (نام جدول/فیلد که در یک‌جا به‌روز شده ولی در جای دیگر نه) بودند که رفع شدند، به‌علاوه یک فیچر کاملاً جدید (دریافت قیمت‌ها) که طبق درخواست شما اضافه شد.
